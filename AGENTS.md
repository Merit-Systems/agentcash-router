...
- **`.metered()` is MPP-only.** Builder throws if `protocols` overrides it to anything else. Requires `RouterConfig.mpp.session` and `mpp.operatorKey`. MPP itself is enabled by `MPP_SECRET_KEY` (or `protocols: ['mpp']`); `MPP_OPERATOR_KEY` does not enable MPP on its own — when MPP is already enabled, setting it auto-enables `session` mode in `createRouterFromEnv`. So `.metered()` needs both `MPP_SECRET_KEY` and `MPP_OPERATOR_KEY`.
