...
/**
 * Build a 402 challenge for a static-priced route. Composes contributions
 * from every allowed payment protocol strategy via `buildChallengeStatic`.
 *
 * Returns:
 ...
 */
export async function buildStatic402(
  ctx: FlowCtx,
  ...
  for (const strategy of getAllowedStrategies(ctx.routeEntry.protocols)) {
    try {
      const contribution = await strategy.buildChallengeStatic({
        request: ctx.request,
