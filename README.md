...
| `MPP_FEE_PAYER_KEY` | no | Sponsors client gas for channel open/topUp. Must resolve to a different address than `MPP_OPERATOR_KEY` (Tempo rejects fee-delegated txs where `sender === feePayer`). **Omit to make clients pay their own gas** — the right default for most paid APIs. **If you set it, the resulting address must hold native Tempo gas before any traffic**, otherwise every paid call fails with a generic `Payment verification failed` HTTP response while the real `insufficient funds for gas` error only surfaces via the `RouterPlugin.onAlert` / `onError` hooks. |

### Other

| Var | Required | Purpose |
|-----|----------|---------|
| `BASE_URL` | yes | Origin URL (`https://api.example.com`). Load-bearing: used as the 402 realm, OpenAPI server URL, and MPP memo prefix. Must match the public domain. **Required at build time, not just runtime** — `next build` collects `/openapi.json` and `/.well-known/x402` by calling into route handlers, and `createRouterFromEnv` validates env eagerly, so a missing value fails the build with `missing_base_url`. On Vercel, use the stable production alias (`<project>.vercel.app` or your custom domain), **not** the per-deployment URL (`<project>-<hash>-<scope>.vercel.app`). |
