...
import { matchPathParams, normalizePath, toHonoPath } from '../src/path-params.js';
...
async function expectNotFoundEnvelope(res: Response) {
  expect(res.status).toBe(404);
  const body = await res.json();
  expect(body.success).toBe(false);
  expect(body.code).toBe('route_not_found');
  expect(body.discovery.openapi).toBe('https://api.example.com/openapi.json');
}
...
describe('router.fetch (Hono dispatch)', () => {
  ...
  it('returns the notFound envelope for unmatched requests', async () => {
    const router = createRouter(baseConfig);
    const res = await router.fetch(
      new Request('https://api.example.com/api/nope', { method: 'POST' }),
    );
    await expectNotFoundEnvelope(res);
  });
  ...
  it('serves discovery surfaces at the root and under the basePath', async () => {
    const router = createRouter(baseConfig);
    router
      .route('search')
      .paid('0.01')
    ...
    expect(JSON.stringify(await wellKnown.json())).toContain('search');
  ...
  it('honors a custom basePath in dispatch and discovery URLs', async () => {
    ...
    expect(openapi.status).toBe(200);
    const doc = await openapi.json();
    expect(doc.paths['/v1/ping']).toBeDefined();
  });

  it('mounts routes at the origin root when basePath is empty', async () => {
    const router = createRouter({ ...baseConfig, basePath: '' });
    router
      .route('ping')
      .unprotected()
      .handler(async () => ({ ok: true }));

    const res = await router.fetch(new Request('https://api.example.com/ping', { method: 'POST' }));
    expect(res.status).toBe(200);
  });

  it('routes a paid route through fetch and gets the 402 challenge', async () => {
    const router = createRouter(baseConfig);
    router
      .route('search')
      .paid('0.01')
    ...
    const res = await router.fetch(
      new Request('https://api.example.com/api/search', { method: 'POST' }),
    );
...
describe('path-params helpers', () => {
  ...
  it('normalizes leading slashes, api/ prefixes, and trailing slashes', () => {
    expect(normalizePath('/v2/search')).toBe('v2/search');
    expect(normalizePath('api/search')).toBe('search');
...
describe('nextHandlers', () => {
  it('returns one delegating handler per method', async () => {
    ...
    await expectNotFoundEnvelope(miss);
  });
});
